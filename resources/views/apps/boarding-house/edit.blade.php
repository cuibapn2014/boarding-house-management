                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <input id="phone" name="phone" class="form-control" type="text" 
                                    placeholder="Nhập số điện thoại liên hệ" value="{{ $boardingHouse?->phone }}" maxlength="10">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4 mb-4">
                <div class="card form-card">
                    <div class="card-body p-4">
                        ...
                        {{-- Đẩy tin lên top: chỉ khi đã publish và chưa đẩy. Đã đẩy thì không cho hoàn. --}}
                        ...
                        <div class="mb-4 p-3 rounded border bg-light">
                            <div class="form-check form-switch mb-2">
                                <input class="form-check-input" type="checkbox" id="push-to-top-toggle" value="1">
                                <label class="form-check-label fw-bold" for="push-to-top-toggle">Đẩy tin lên top</label>
                            </div>
                            <p class="text-muted small mb-2">Chọn thời gian hiển thị ở đầu danh sách (trừ điểm).</p>
                            <div id="push-duration-wrapper" class="mb-2" style="display: none;">
                                <label class="form-label small">Thời gian hiển thị <span class="text-danger">*</span></label>
                                <select id="push-listing-days" class="form-control form-control-sm">
                                    <option value="">Chọn số ngày</option>
                                    @foreach($pushDurationOptions ?? [] as $days => $points)
                                    <option value="{{ $days }}" data-points="{{ $points }}">{{ $days }} ngày — {{ $points }} point</option>
                                    @endforeach
                                </select>
                            </div>
                            <button type="button" id="btn-push-listing" class="btn btn-warning btn-sm" style="display: none;"
                                data-url="{{ route('boarding-house.push', [$boardingHouse->id]) }}">
                                <i class="fas fa-arrow-up me-1"></i>Đẩy tin
                            </button>
                        </div>
                        @endif

                        <div class="mb-4 p-3 rounded bg-light border">
                            <span class="form-label mb-0 d-block">Số dư điểm</span>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const pushToggle = document.getElementById('push-to-top-toggle');
        const pushDurationWrapper = document.getElementById('push-duration-wrapper');
        const pushListingDays = document.getElementById('push-listing-days');
        const btnPushListing = document.getElementById('btn-push-listing');

        if (pushToggle && pushDurationWrapper && btnPushListing) {
            pushToggle.addEventListener('change', function() {
                const show = this.checked;
                pushDurationWrapper.style.display = show ? 'block' : 'none';
                btnPushListing.style.display = show ? 'inline-block' : 'none';
                if (!show && pushListingDays) pushListingDays.value = '';
            });
        }

        if (btnPushListing) {
            btnPushListing.addEventListener('click', function() {
                const listingDays = pushListingDays ? pushListingDays.value : '';
                if (!listingDays) {
                    alert('Vui lòng chọn thời gian hiển thị (10, 15, 30 hoặc 60 ngày).');
                    if (pushListingDays) pushListingDays.focus();
                    return;
                }
                const opt = pushListingDays && pushListingDays.options[pushListingDays.selectedIndex];
                const points = opt ? (opt.dataset.points || '') : '';
                if (!confirm('Đẩy tin lên đầu danh sách trong ' + listingDays + ' ngày (trừ ' + points + ' point)?')) return;
                ...
                fetch(url, {
                    ...
                    body: JSON.stringify({ listing_days: parseInt(listingDays, 10) })
                })
                .then(r => r.json())
                .then(data => {
                ...
                .finally(function() { btnPushListing.disabled = false; });
            });
        }
    });
</script>
